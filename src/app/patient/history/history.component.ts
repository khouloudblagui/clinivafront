...
export class HistoryComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (!currentUser.id) {
      console.error("Invalid currentUser in localStorage: Missing ID");
      this.showNotification('snackbar-error', 'Missing patient information. Please login again.', 'top', 'center');
      return;
    }
    this.patientId = currentUser.id;
    console.log("Patient ID:", this.patientId); // Log pour vérifier
  }
  ...
  onSubmit(): void {
    if (this.historyForm.valid) {
      ...
      const newHistory: UserHistory = {
        his_ky: 0, // Initialisez à 0 ou null si c'est un nouvel historique
        patient: this.patientId, // Ajout correct de l'ID du patient
        q1: formData.q1,
        q2: formData.q2,
        q3: formData.q3,
        q4: formData.q4,
